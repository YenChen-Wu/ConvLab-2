...
#                                  nn.Flatten(), nn.Linear(3136, config.n_embd), nn.Tanh())
        self.state_encoder = nn.Sequential(nn.Linear(340, config.n_embd), nn.Tanh())
...
#             print (states.shape, actions.shape, rtgs.shape, timesteps.shape)
        ...
        state_embeddings = self.state_encoder(states.type(torch.float32).contiguous())
...
#             action_embeddings = self.action_embeddings(actions.type(torch.long).squeeze(-1)) # (batch, block_size, n_embd)
            action_embeddings = self.action_embeddings(actions.type(torch.float32)) \
                                .reshape(actions.shape[0], actions.shape[1], self.config.n_embd) 
...
#         print(torch.gather(all_global_pos_emb, 1, torch.repeat_interleave(timesteps, self.config.n_embd, dim=-1)).shape, self.pos_emb[:, :token_embeddings.shape[1], :].shape)
...
#             loss = F.cross_entropy(logits.reshape(-1, logits.size(-1)), targets.reshape(-1))
            loss = F.binary_cross_entropy_with_logits(logits.reshape(-1, logits.size(-1)), targets.type(torch.float32).reshape(-1, targets.size(-1)))
